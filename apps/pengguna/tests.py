...
class PenggunaViewTests(TestCase):
    ...
    def test_detail_pengguna_tidak_menampilkan_tombol_hapus(self):
        ...
        self.assertContains(response, 'html[data-theme="dark"] .profile-page .surface-card')
        self.assertContains(response, 'html[data-theme="dark"] [data-profile-modal] > div')
        self.assertContains(response, 'profile-edit-modal')
        self.assertContains(response, 'html[data-theme="dark"] [data-profile-modal] [data-file-upload] > div:not(.sr-only)')
        self.assertContains(response, 'html[data-theme="dark"] [data-file-upload] .file-upload-dropzone')
        self.assertContains(response, 'background: rgba(15, 23, 42, 0.58) !important;')
        self.assertContains(response, 'html[data-theme="dark"] [data-file-upload] .file-upload-button')
        self.assertContains(response, 'color: #99f6e4 !important;')
        self.assertContains(response, 'html[data-theme="dark"] [data-file-upload] .file-upload-title')
        self.assertContains(response, 'html[data-theme="dark"] [data-file-upload] .file-upload-name')
        self.assertContains(response, 'data-crop-enabled="true"')
        ...
        self.assertContains(response, 'function openCropper(component, file, input)')
        self.assertContains(response, 'setFileInputFile(input, croppedFile)')
        self.assertContains(response, 'html[data-theme="dark"] [data-profile-modal] input:focus')
        self.assertContains(response, 'experience-profile-modal')
        self.assertContains(response, 'data-experience-modal-open')
        self.assertContains(response, 'experience-category-panel')
        self.assertContains(response, 'Tambah Profil Profesional')
...
class PenggunaAuthTests(TestCase):
    ...
    def test_register_fakultas_dan_prodi_berupa_dropdown(self):
        ...
        self.assertContains(response, 'name="no_hp"', html=False)
        self.assertContains(response, 'minlength="10"', html=False)
        self.assertContains(response, "querySelectorAll('#id_nim_nik, #id_no_hp')", html=False)
    ...
    def test_register_tidak_menampilkan_input_email_dan_menjelaskan_email_dari_nim(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, 'id="id_email"', html=False)
        self.assertContains(response, 'data-derived-email', html=False)
        self.assertContains(response, '@std.trisakti.ac.id')
        self.assertContains(response, 'Data akun')
        self.assertContains(response, 'Data akademik')
        self.assertContains(response, 'Profil')
        content = response.content.decode()
        account_section = content.split('id="register-account-title"', 1)[1].split('</section>', 1)[0]
        expected_order = [
            'id_nama_pengguna',
            'id_nim_nik',
            'id_no_hp',
        ...
        positions = [account_section.index(field_id) for field_id in expected_order]
    ...
    @patch('apps.pengguna.forms.validate_human_face_photo')
    def test_register_membuat_email_dari_nim_dan_mengabaikan_email_kiriman_klien(self, _mock_validate_face):
        response = self.client.post(
            reverse('pengguna:register'),
            {
                'nama_pengguna': 'Email Otomatis',
                'nim_nik': '0642201099',
                'email': 'alamat.palsu@gmail.com',
                'password': 'passwordku123',
                'password_confirmation': 'passwordku123',
                'no_hp': '081234567899',
                'alamat': 'Jakarta',
        ...
        self.assertRedirects(response, reverse('pengguna:verify_register'))
        self.assertTrue(Pengguna.objects.filter(email='0642201099@std.trisakti.ac.id').exists())
        self.assertFalse(Pengguna.objects.filter(email='alamat.palsu@gmail.com').exists())
    ...
    def test_register_menolak_email_turunan_nim_yang_sudah_terdaftar(self):
        Pengguna.objects.create(
            nama_pengguna='Pemilik Email',
            nim_nik='2201999',
            email='0642201010@std.trisakti.ac.id',
            password='rahasia123',
            no_hp='081111111111',
            alamat='Jakarta',
            fakultas='Teknologi Industri',
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'NIM ini sudah memiliki akun')
